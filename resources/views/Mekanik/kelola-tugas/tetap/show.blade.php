...
<div class="bg-white rounded-xl shadow p-4 sm:p-6">
    ...
    <div class="space-y-3 text-sm sm:text-base">
        ...
        <div class="flex flex-col sm:flex-row sm:gap-2">
            <strong class="min-w-[140px]">Pemberi Tugas:</strong>
            <span>{{ $tugas->pemberi_tugas }}</span>
        </div>

        <div class="flex flex-col sm:flex-row sm:gap-2">
            <strong class="min-w-[140px]">Equipment:</strong>
            <span class="break-words">{{ $tugas->equipment }}</span>
        </div>

        <div class="flex flex-col sm:flex-row sm:gap-2">
            <strong class="min-w-[140px]">Lokasi:</strong>
            <span class="break-words">{{ $tugas->lokasi }}</span>
        </div>

        <div class="flex flex-col sm:flex-row sm:gap-2">
            <strong class="min-w-[140px]">Jenis Tugas:</strong>
            <span>{{ ucfirst($tugas->jenis_tugas) }}</span>
        </div>

        <div class="flex flex-col sm:flex-row sm:gap-2">
            <strong class="min-w-[140px]">Tanggal Mulai:</strong>
            <span>
                @if($tugas->jenis_tugas == 'mingguan')
                    Setiap {{ ucfirst($tugas->hari_mingguan) }}
                @elseif($tugas->jenis_tugas == 'bulanan')
                    Tanggal {{ $tugas->tanggal_bulanan }}
        ...
        <div class="flex flex-col sm:flex-row sm:gap-2 items-start sm:items-center">
            <strong class="min-w-[140px]">Status:</strong>
