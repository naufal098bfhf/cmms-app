...
<div class="p-6 bg-white rounded-2xl shadow-lg">
    <h2 class="text-xl md:text-2xl font-bold text-gray-700 mb-4 md:mb-6">Kelola Tugas Darurat</h2>

    {{-- Form pencarian --}}
    <form method="GET" action="{{ route('maintenance-planning.kelola-tugas.tugas-darurat.index') }}" class="flex flex-wrap gap-3 mb-5">
        ...
        <a href="{{ route('maintenance-planning.kelola-tugas.tugas-darurat.create') }}" class="bg-rose-300 text-white px-4 py-2 rounded-lg hover:bg-rose-400 ml-auto">Tambah Baru</a>
    </form>
    ...
    <div class="overflow-x-auto">
        <table class="w-full min-w-[900px] border border-gray-200 text-xs md:text-sm text-left">
                    ...
                    <tr class="hover:bg-rose-50">
                      ...
                      <td class="p-2 border text-center">
                        @if($tugas->status == 'pending')
                            <span class="bg-yellow-200 text-yellow-800 px-2 py-1 rounded text-xs font-semibold">Release Order</span>
                        @elseif($tugas->status == 'dikerjakan')
                            <span class="bg-blue-200 text-blue-800 px-2 py-1 rounded text-xs font-semibold">Dikerjakan</span>
                        @elseif($tugas->status == 'selesai')
                            <span class="bg-green-200 text-green-800 px-2 py-1 rounded text-xs font-semibold">Selesai</span>
                        @else
    ...
    <div class="flex justify-center items-center gap-3">
                            {{-- Edit --}}
                            <a href="{{ route('maintenance-planning.kelola-tugas.tugas-darurat.edit', $tugas->id) }}"
                            ...
                            {{-- Hapus --}}
                            <form method="POST" action="{{ route('maintenance-planning.kelola-tugas.tugas-darurat.destroy', $tugas->id) }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
