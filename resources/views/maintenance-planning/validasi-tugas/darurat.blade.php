...
<div class="p-6 bg-white rounded-2xl shadow-md">
    <h2 class="text-2xl font-semibold text-gray-700 mb-4">Validasi Tugas Darurat</h2>

    <div class="border p-4 rounded-lg bg-gray-50 space-y-2">
        <p><strong>Mekanik:</strong> {{ $tugas->mekanik->name }}</p>
        <p><strong>Equipment:</strong> {{ $tugas->equipment ?? $tugas->equipment_id ?? '-' }}</p>
    ...
    <form method="POST" action="{{ route('maintenance-planning.validasi-tugas.darurat.update', $tugas->id) }}" class="mt-5">
        @csrf
        @method('PUT')
